...
# COPY OF code1VScode2_LE.py
# MAKE THE MESSAGE CENTERED (NG PRESSURE OR NO DATA FOUND)
# REMOVE THE DISPLAYED DATA DUE TO "NG PRESSURE OR NO DATA FOUND" DISPLAY
# CREATE A ROW COUNTER SO THAT IT WILL NOT READ THE TAIL TWICE
# LINE TREND COMPRESS (1,357 ROWS PER DATE)
# MANUAL CHECK OF AVERAGE DONE
# REMOVE (DONE): EVERY NEW DAY FIRST RUN SERIAL WILL HAVE NO REFERENCE, AND THE DEVIATION WILL ALL BE EQUAL TO 5% (THIS MUST BE DELETED)
...
# File path  
# file_path = r"\\192.168.2.19\ai_team\AI Program\Outputs\CompiledPiMachine\CompiledPIMachine.csv"  # Network path
# file_path = r"C:\Users\ai.pc\OneDrive\Desktop\CompiledPIMachine.csv"  # Testing path
...
class DatabaseSelection:
    def __init__(self, root):
        ...
        self.root.geometry("450x310")   # 450x300 ORIGINAL SIZE
    ...
    def confirm_selection(self):
        ...
        try:
            ...
            threshold = 3.0  # Default to 3% if invalid
        if location:
            self.root.destroy()
            root = tk.Tk()
...
class FluctuationMonitor:
    def __init__(self, root, location, threshold=3.0):
        self.root = root
        self.root.title("Historical Measurement Trend")
        ...
        # if location == "HP1":
        #     self.file_path = r"\\192.168.2.19\general\INSPECTION-MACHINE\HP2\log508_ATU006_2508.csv"
        #     self.output_path = r"\\192.168.2.19\general\INSPECTION-MACHINE\HP2\fluctuatedQC_CSV.csv"
        # elif location == "HP2":
        #     self.file_path = r"\\192.168.2.19\general\INSPECTION-MACHINE\HP1\log008_ATU003_2508.csv"
        ...
        # Commented out to prevent "PREVIOUS REF SN:" from being displayed
    ...
    def zoom(self, factor):
        ...
        widgets = [
            ...
            # (self.ref_serial_display, 12),  # Commented out since ref_serial_display is not created
        ]
        ...
        if hasattr(self, 'line_fig'):
            # Adjust line graph size based on window size and zoom level
            window_width = self.root.winfo_width()
            window_height = self.root.winfo_height()
            # Calculate figure size to nearly fill the window width and a portion of height
            fig_width = max(8, window_width / 100 * 0.95) * self.zoom_level  # 95% of window width
            fig_height = max(3, window_height / 100 * 0.4) * self.zoom_level  # 40% of window height
    ...
    def create_bar_graph(self):
        ...
        self.ax.set_ylabel('FluctDeviation Amount (%)')
        self.ax.grid(True)
    ...
    def create_line_graph(self):
        # Calculate initial figure size based on window size
        window_width = self.root.winfo_width()
        window_height = self.root.winfo_height()
        # Set figure size to nearly fill the window width and a portion of height
        fig_width = max(8, window_width / 100 * 0.95)  # 95% of window width
        fig_height = max(3, window_height / 100 * 0.4)  # 40% of window height
        ...
        try:
            ...
            for date, group in df.groupby(df['DATE'].dt.date):
                ...
                if n > 30:
                    ...
                    selected = group
                compressed_df = pd.concat([compressed_df, selected], ignore_index=True)
            df = compressed_df
    ...
    def update_bar_graph(self, last_row):
        try:
            ...
            self.bars = self.ax.bar(self.fluctuation_measurements, self.fluctuation_values, color='skyblue')
            self.ax.set_title('Current Deviation Values')
            self.ax.set_ylabel('Deviation Amount (%)')
            self.ax.grid(True)
            self.ax.set_xticks(range(len(self.fluctuation_measurements)))
            self.ax.set_xticklabels(self.fluctuation_measurements, rotation=45, ha='right', fontsize=7)
    ...
    def process_and_update(self):
        ...
        try:
            ...
            if self.previous_row_count > 0 and current_row_count <= self.previous_row_count:
                self.status_box.configure(bg="red")
                self.status_box.delete("all")  # Clear existing text
                self.status_text = self.status_box.create_text(150, 50, text="NG PRESSURE", 
                                                             font=('Arial', 12, 'bold'), fill="white", anchor="center")
                self.status_box.create_text(150, 75, text="OR", 
                                          font=('Arial', 12, 'bold'), fill="white", anchor="center")
                self.status_box.create_text(150, 100, text="NO DATA FOUND", 
                                          font=('Arial', 12, 'bold'), fill="white", anchor="center")
                # Clear display elements
                self.serial_display.config(text="SERIAL No.: N/A")
                self.model_display.config(text="MODEL CODE: N/A")
            ...
            wattageMiddle_avg_map = model_summary.set_index("MODEL CODE")["WATTAGE Middle AVG"].to_dict()
            amperageMiddle_avg_map = model_summary.set_index("MODEL CODE")["AMPERAGE Middle AVG"].to_dict()
            closePressureMiddle_avg_map = model_summary.set_index("MODEL CODE")["CLOSED PRESSURE Middle AVG"].to_dict()
            voltageMin_avg_map = model_summary.set_index("MODEL CODE")["VOLTAGE MIN (V) AVG"].to_dict()
            wattageMin_avg_map = model_summary.set_index("MODEL CODE")["WATTAGE MIN AVG"].to_dict()
            closePressureMin_avg_map = model_summary.set_index("MODEL CODE")["CLOSED PRESSURE MIN AVG"].to_dict()
            previous_values = {
                'V_MAX': None,
                'W_MAX': None,
                'CP_MAX': None,
            ...
            for a in range(len(df)):
                ...
                if model_changed:
                    ...
                    # self.last_good_values = {}  # Removed to maintain reference values
                    # self.last_good_serial = None  # Removed to maintain reference serial
                if is_new_date:
                ...
                dataFrame["AVE WATTAGE Middle (W)"] = wattageMiddle_avg_map.get(model_code)
                dataFrame["AVE AMPERAGE Middle (A)"] = amperageMiddle_avg_map.get(model_code)
                dataFrame["AVE CLOSED PRESSURE Middle (kPa)"] = closePressureMiddle_avg_map.get(model_code)
                dataFrame["AVE VOLTAGE MIN (V)"] = voltageMin_avg_map.get(model_code)
                dataFrame["AVE WATTAGE MIN (W)"] = wattageMin_avg_map.get(model_code)
                dataFrame["AVE CLOSED PRESSURE MIN (kPa)"] = closePressureMin_avg_map.get(model_code)
                current_values = {
                    'V_MAX': tempdf["VOLTAGE MAX (V)"].values[0],
                    'W_MAX': tempdf["WATTAGE MAX (W)"].values[0],
                    'CP_MAX': tempdf["CLOSED PRESSURE_MAX (kPa)"].values[0],
                    'V_MID': tempdf["VOLTAGE Middle (V)"].values[0],
                    'W_MID': tempdf["WATTAGE Middle (W)"].values[0],
                    'A_MID': tempdf["AMPERAGE Middle (A)"].values[0],
                    'CP_MID': tempdf["CLOSED PRESSURE Middle (kPa)"].values[0],
                    'V_MIN': tempdf["VOLTAGE MIN (V)"].values[0],
                    'W_MIN': tempdf["WATTAGE MIN (W)"].values[0],
                    'CP_MIN': tempdf["CLOSED PRESSURE MIN (kPa)"].values[0]
                }
                fluctuations = {}
                for key in current_values:
                    if current_values[key] == 0:
                        ...
                        if pd.notnull(avg_value) and avg_value != 0:
                            fluctuations[key] = (current_values[key] - avg_value) / avg_value
                        else:
                            fluctuations[key] = 0
    ...
    def update_display(self):
        try:
            ...
            # if "REFERENCE SERIAL" in last_row:  # Commented out to avoid updating non-existent label
            ...
            # logging.info("\nLast 5 rows of compiledFrame:")     # Commented to prevent DataFrame display in console
            # logging.info(self.compiledFrame.tail(5))            # Commented to prevent DataFrame display in console
        except Exception as e:
    ...
    def reset_fluctuation(self, column_name):
        try:
            ...
            if all(abs(last_row[col]) <= self.threshold for col in self.status_vars if col in last_row):
                ...
                # self.ref_serial_display.config(text=f"PREVIOUS REF SN: {self.last_good_serial}")  # Commented out to avoid updating non-existent label
            self.compiledFrame.to_csv(self.output_path, index=False, encoding='utf-8-sig')      # Save to CSV
            self.update_display()
        except Exception as e:
